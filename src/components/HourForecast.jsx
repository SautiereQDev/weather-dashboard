    ...
    <ErrorBoundary>
      ...
      <div className="flex gap-4 justify-evenly mt-6 mx-4">
        {weather.list && weather.list.length > 0 ? (
          [...Array(9)].map((_, i) => <HourCard key={i} index={i} day={day} />)
        ) : (
      ...
      <div className="flex justify-between mt-5 mx-4">
        <button
          className={day > 0 ? 'p-2 bg-blue-700 text-white rounded-lg border' : 'p-2 invisible'}
          onClick={() => setDay(day - 1)}
        >
          Jour précedent
        </button>
        <button
          className={day < 3 ? 'p-2 bg-blue-700 text-white rounded-lg border' : 'p-2 invisible'}
          onClick={() => setDay(day + 1)}
        >
          Jour suivant
        </button>
